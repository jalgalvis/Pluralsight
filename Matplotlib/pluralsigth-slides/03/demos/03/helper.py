...
def precip_sums_for_year(year=2018):
    ...
    return sorted(zip(weather_for_year.keys(),
                      [sum([day['precipitation'] for day in weather_for_year[month]]) for month in
                       weather_for_year.keys()]))
...
def temps_by_month_for_year(year=2018):
    ...
    the_weather = dict(zip(sorted(the_weather.keys()),
                           [[day['temperaturemax'] for day in the_weather[month]] for month in
                            sorted(the_weather.keys())]))
